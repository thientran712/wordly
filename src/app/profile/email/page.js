    ...
    <div ref={ref} style={{ position: "relative", flex: 1 }}>
      <div
        ...
        style={{
          ...
          border: `1.5px solid ${open ? "rgba(var(--electric-rgb),0.5)" : "var(--input-border)"}`,
      ...
      <main className="relative z-10 max-w-xl mx-auto px-4 sm:px-8 py-6 sm:py-8 pb-16">
        {error && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[200] w-[90vw] max-w-xl px-5 py-4 rounded-2xl shadow-xl text-sm font-semibold"
            style={{ background: "rgba(248,113,113,0.12)", border: "1px solid rgba(248,113,113,0.3)", color: "var(--error)" }}>
            ⚠️ {error}
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setEmailEnabled(prev => !prev)}
                ...
                <span className="absolute top-0.5 w-5 h-5 rounded-full shadow transition-all"
                  style={{ left: emailEnabled ? "26px" : "2px", background: emailEnabled ? "var(--on-electric)" : "var(--surface-elevated)" }} />
              </button>
            </div>
          )}
        </div>
                ...
                {[
                  ...
                  <button type="button" key={f.value} onClick={() => setFrequency(f.value)}
                    ...
                    style={{
                      ...
                      color: frequency === f.value ? "var(--on-electric)" : "var(--ink-soft)",
                      boxShadow: frequency === f.value ? "0 4px 12px rgba(var(--electric-rgb),0.25)" : "none",
                    }}>
                    {f.label}
                  </button>
                ))}
                  ...
                  {["CN", "T2", "T3", "T4", "T5", "T6", "T7"].map((day, i) => (
                    <button type="button" key={i} onClick={() => toggleCustomDay(i)}
                      ...
                      style={{
                        ...
                        color: customDays.includes(i) ? "var(--on-electric)" : "var(--ink-soft)",
                        boxShadow: customDays.includes(i) ? "0 4px 12px rgba(var(--electric-rgb),0.25)" : "none",
                      }}>
                      {day}
                    </button>
                  ))}
        ...
        <button type="button" onClick={handleSavePrefs} disabled={isSaving || isLoading}
          ...
          style={{
            background: success ? "rgba(var(--electric-rgb),0.15)" : "var(--electric)",
            color: success ? "var(--electric)" : "var(--on-electric)",
            border: success ? "1.5px solid rgba(var(--electric-rgb),0.4)" : "none",
